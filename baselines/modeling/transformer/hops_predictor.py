...
from .hops_model import Aggregator
#gs
from baselines.third_party import clip
# from baselines.tagclip import clip
...
class HopsPredictor(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if clip_pretrained == "ViT-G" or clip_pretrained == "ViT-H":
            ...
            # clip_model, clip_preprocess = clip.load(clip_pretrained, device=device)
        ...
        # self.text_part_features = self.class_embeddings(
        #     self.train_part_classes, prompt_templates, clip_model
        # ).permute(1, 0, 2).float()
    ...
    def forward(self, x, vis_guidance, AFM_A , AFM=False, test_text=None, conf=False):

        # TODO: 2024.11.04
        # vis_guidance
        vis = [vis_guidance[k] for k in vis_guidance.keys()][::-1]
        # vis[0].shape: torch.Size([4, 1024, 24, 24])->([4, 384, 24, 24])
        # vis[1].shape: torch.Size([4, 512, 48, 48])->([4, 384, 48, 48])
        # vis[2].shape: torch.Size([4, 256, 96, 96])->([4, 384, 96, 96])

        if self.training:
            part_obj_text = self.text_part_obj_features # torch.Size([53, 80, 512]) 53是part类别数+物体类别数+背景
            specific_part_text = self.text_specific_part_features #torch.Size([75, 80, 512]) 74 specific part + background
            part_index_map = self.train_text_to_part_map[None, None, None, :len(self.train_text_classes)] # 具体obj-part对应part的index
            obj_index_map = self.train_text_to_obj_in_part_map[None, None, None, :]# 具体obj-part对应obj的index
        ...
        part_obj_text = part_obj_text.repeat(x.shape[0], 1, 1, 1) # 41part + 11obj + background
        specific_part_text = specific_part_text.repeat(x.shape[0], 1, 1, 1)
        obj_out = self.transformer(
            x, 
            part_obj_text, 
            specific_part_text, 
        ...
        # part_obj_out, specific_part_out, costs = self.transformer(
        #     x, 
        #     part_obj_text, 
        #     specific_part_text, 
        #     vis,  # appearance guidance
        ...
        #     num_part_classes
        # )
        return obj_out #obj级别的logit
    
        #return part_obj_out, specific_part_out, costs
    
    def forward_second(self, x, vis_guidance, part_in_my_obj, AEM_A,AEM=False,test_text=None):#第二阶段,返回part
        ...
        # part_text = part_text.repeat(x.shape[0], 1, 1, 1)
        specific_part_text = specific_part_text.repeat(x.shape[0], 1, 1, 1)
        # part_text = part_text.repeat(x.shape[0], 1, 1, 1)
        # 只保留已经预测出的obj的part
        # part_text_select = specific_part_text[:, part_in_my_obj, :, :]
        part_text_select = part_obj_text[:, part_in_my_obj, :, :] # 提取对应的part文本特征
        # part_text_select = specific_part_text[:, part_in_my_obj, :, :] # 提取对应的specific part文本特征
        part_out = self.transformer.forward_second_stage(
            x, 
            part_obj_text,
            part_text_select, 
            vis,  # appearance guidance
