...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, appearance_guidance):
        ...
        if appearance_guidance is not None:
            ...
            x = torch.cat([x, appearance_guidance], dim=-1) #costemb与dino拼接(先前全128维) torch.Size([12, 24, 24, 256])
    ...
    def forward(self, x, appearance_guidance):
        ...
        if appearance_guidance is not None:
            appearance_guidance = self.guidance_norm(
                repeat(appearance_guidance, 'B C H W -> (B T) (H W) C', T=T)) #重复了12次 torch.Size([12, 576, 128])
...
class QueryAttention(nn.Module):
    def __init__(self, use_dropout=False, attention_dropout=0.1, eps=1e-8):
        super().__init__()
        self.use_dropout = use_dropout
        self.dropout = nn.Dropout(attention_dropout)
        self.eps = eps
    ...
    def forward(self, queries, keys, values, q_mask=None, kv_mask=None):
        ...
        # Compute the attention and the weighted average
        softmax_temp = 1. / queries.size(3)**.5  # sqrt(D)
        A = (softmax_temp * QK).softmax(dim=1) + self.eps
        ...
        # queried_values = torch.einsum("nlsh,nshd->nlhd", A, values)
        queried_values = torch.einsum("nshd,nlsh->nlhd", values, A)
...
class AttentionLayer(nn.Module):
    def __init__(self, hidden_dim, guidance_dim, nheads=8, attention_type='linear'):
        ...
        elif attention_type == 'query':
            self.attention = QueryAttention()
        else:
...
class AggregatorLayer(nn.Module):
    ...
    def forward(self, x, appearance_guidance=None, text_guidance=None):
        """
        Arguments:
            x: B C T H W
        """
...
class Aggregator(nn.Module):
    def __init__(self,
        ...
        self.layers = nn.ModuleList([
            AggregatorLayer(
                ...
                nheads=nheads, input_resolution=feature_resolution, pooling_size=pooling_size, window_size=window_size, attention_type="query", # attention_type=attention_type,
            ) for _ in range(num_layers)
        ])

        self.layers_object = nn.ModuleList([
            AggregatorLayer(
                hidden_dim=hidden_dim, text_guidance_dim=text_guidance_proj_dim, appearance_guidance=appearance_guidance_proj_dim,
                nheads=nheads, input_resolution=feature_resolution, pooling_size=pooling_size, window_size=window_size, attention_type="query", # attention_type=attention_type,
            ) for _ in range(num_layers)
        ...
        self.layers_specific_part = nn.ModuleList([
            AggregatorLayer(
                hidden_dim=hidden_dim, text_guidance_dim=text_guidance_proj_dim, appearance_guidance=appearance_guidance_proj_dim,
                nheads=nheads, input_resolution=feature_resolution, pooling_size=pooling_size, window_size=window_size, attention_type="query", # attention_type=attention_type,
            ) for _ in range(num_layers)
        ])
        self.layers_part = nn.ModuleList([
            AggregatorLayer(
                hidden_dim=hidden_dim, text_guidance_dim=text_guidance_proj_dim, appearance_guidance=appearance_guidance_proj_dim,
                nheads=nheads, input_resolution=feature_resolution, pooling_size=pooling_size, window_size=window_size, attention_type="query", # attention_type=attention_type,
            ) for _ in range(num_layers)
        ...
        self.conv2 = nn.Conv2d(prompt_channel, hidden_dim,
                               kernel_size=7, stride=1, padding=3)
        
        self.conv_proj_feat = nn.Conv2d(hidden_dim * 2, text_guidance_dim, kernel_size=1)
        ...
        self.decoder1_part = Up(
            hidden_dim, decoder_dims[0], decoder_guidance_proj_dims[0])
        self.decoder2_part = Up(
            decoder_dims[0], decoder_dims[1], decoder_guidance_proj_dims[1])
        self.head_part = nn.Conv2d(
            decoder_dims[1], 1, kernel_size=3, stride=1, padding=1)
    ...
    def correlation(self, img_feats, text_feats):
        img_feats = F.normalize(img_feats, dim=1)  # B C H W
        text_feats = F.normalize(text_feats, dim=-1)  # B T P C
        corr = torch.einsum('bchw, btpc -> bpthw', img_feats, text_feats)  # B P T H W
        return corr
    
    def correlation_cost(self, cost_feats, text_feats):
        cost_feats = F.normalize(cost_feats, dim=2)  # B T C H W
        text_feats = F.normalize(text_feats, dim=-1)  # B T P C
        corr = torch.einsum('btchw, btpc -> bpthw', cost_feats, text_feats)
        return corr
    ...
    def conv_decoder_part(self, x, guidance):
        B = x.shape[0]
        corr_embed = rearrange(x, 'B C T H W -> (B T) C H W')
        corr_embed = self.decoder1_part(corr_embed, guidance[0])
        corr_embed = self.decoder2_part(corr_embed, guidance[1])
        corr_embed = self.head_part(corr_embed)
        corr_embed = rearrange(corr_embed, '(B T) () H W -> B T H W', B=B)
        return corr_embed
    ...
    def forward(self, 
        ...
        """
        # Cost Aggregation for Generalized Parts and Objects
        text_obj_feats = text_part_obj_feats[:,num_part_classes:,:]  # torch.Size([1, 12, 80, 512])
        corr = self.correlation(img_feats, text_obj_feats) # torch.Size([1, 80, 12, 24, 24])
        
        # 挑选出12层中注意力值大于均值4次的位置与原cost做相乘
        if(AFM):
            AFM_A_mean = AFM_A.mean(dim=[-1,-2], keepdim=True)   # 形状 [1, 12, 1, 1]
            Attn_vote = AFM_A > AFM_A_mean
            attn_cnt = Attn_vote.sum(dim=1)
            attn_cnt = attn_cnt > 4 

            AFM_A = AFM_A.mean(dim = 1)
            AFM_A = AFM_A * attn_cnt

            B, T, C, H, W = corr.shape
            N = H * W                           # 576
            corr_flat = corr.view(B, T, C, N, 1).clone()
            corr_refine = torch.zeros_like(corr_flat)
            for b in range(B):
                corr_refine[b] = torch.matmul(AFM_A[b], corr_flat[b])
            corr_refine = corr_refine.view(B, T, C, H, W)
            corr = corr_refine
            # corr_refine = corr_refine.mean(dim=1)
            
            # corr = corr.view(B, T, C, N, 1)
            # for b in range(B):
            #     for c in range(C):
            #         aff_mask = torch.zeros(H,W)
            #         temp_corr = corr_refine[b,c,:,:]
            #         aff_mask = temp_corr > temp_corr.mean()
            #         aff_mask = aff_mask.view(1,H*W)
            #         trans_mat = AFM_A * aff_mask
            #         corr[b,:,c] = torch.matmul(trans_mat, corr[b,:,c])
            # corr = corr.view(B, T, C, H, W)
        
        corr_embed = self.corr_embed(corr) # torch.Size([1, 128, 12, 24, 24])
        ...
        if self.guidance_projection is not None:
            # appearance_guidance[0]  # 4, 1024, 24, 24
            # appearance_guidance[0]  # 4, 1408, 24, 24  # 1024+384
            # projected_guidance  # 4, 128, 24, 24
            projected_guidance = self.guidance_projection(appearance_guidance[0]) # torch.Size([1, 128, 24, 24])
        if self.decoder_guidance_projection is not None:
            projected_decoder_guidance = [
                proj(g)
                for proj, g
                in zip(self.decoder_guidance_projection, appearance_guidance[1:])
            ]
            # projected_decoder_guidance[0].shape: torch.Size([4, 32, 48, 48])
            # projected_decoder_guidance[1].shape: torch.Size([4, 16, 96, 96])

        if self.text_guidance_projection is not None:
            text_obj_feats = text_obj_feats.mean(dim=-2)
            text_obj_feats = text_obj_feats / text_obj_feats.norm(dim=-1, keepdim=True)
            projected_text_guidance = self.text_guidance_projection(text_obj_feats) # torch.Size([1, 12, 128])
        ...
        for i, (layer, layer_object) in enumerate(zip(self.layers, self.layers_object)):
            ...
            agg_output = layer(corr_embed, projected_guidance, projected_text_guidance) # 聚合后的输出结果torch.Size([1, 128, 12, 24, 24])
            # part_corr_embed, obj_corr_embed = agg_output[:, :, :num_part_classes], agg_output[:, :, num_part_classes:]

            # corr_embed = torch.cat([part_corr_embed, obj_corr_embed], dim=2)
        obj_logit = self.conv_decoder(agg_output, projected_decoder_guidance) # torch.Size([1, 12, 96, 96])

        return obj_logit
    
    def sinkhorn(self, M, max_iter=5): #获得D矩阵
        for _ in range(max_iter):
            M = M / M.sum(dim=1, keepdim=True)   # 行和 = 1
            M = M / M.sum(dim=0, keepdim=True)   # 列和 = 1
        return M
    def forward_second_stage(self, #第二阶段,part聚合
                img_feats, 
                text_part_obj_feats, 
                text_part_select, 
                appearance_guidance,
        ...
        """
        # Cost Aggregation for Generalized Parts and Objects
        text_part_feats = text_part_select
        corr = self.correlation(img_feats, text_part_feats)
        if(AEM):
            
            corr_refine = torch.zeros_like(corr)
            AEM_A = torch.mean(AEM_A, dim=1).squeeze(0)
            AEM_D = self.sinkhorn(AEM_A)
            M_aff = 0.5*(AEM_D + AEM_D.T)
            B,T,C,H,W = corr.shape
            corr = corr.view(B, T, C, H*W, 1)
            corr = torch.matmul(M_aff, corr)
            corr = corr.view(B, T, C, H, W)
            # for t in range(T):
            #     for c in range(C):
            #         aff_mask = torch.zeros(H,W)
            #         temp_corr = corr[0][t][c]
            #         box, cnt = self.scoremap2bbox(scoremap=temp_corr.detach().cpu().numpy(), threshold=0.6, multi_contour_eval=True)
            #         for i_ in range(cnt):
        ...
        # Part Normalization
        # part_corr = corr[:, :, :num_part_classes]
        # norm_part_corr = F.softmax(part_corr, dim=2)
        # corr[:, :, :num_part_classes] = norm_part_corr
        ...
        if self.text_guidance_projection is not None:
            text_part_feats = text_part_feats.mean(dim=-2)
            text_part_feats = text_part_feats / text_part_feats.norm(dim=-1, keepdim=True)
            projected_text_guidance = self.text_guidance_projection(text_part_feats)

        for i, (layer, layer_object) in enumerate(zip(self.layers_part, self.layers_specific_part)):
            # if i == len(self.layers) - 1:
            #     part_corr_embed = layer(corr_embed, projected_guidance, projected_text_guidance)[:, :, :num_part_classes]
            #     # part_corr_embed = layer(corr_embed, text_guidance=projected_text_guidance)[:, :, :num_part_classes]
            # else:
            ...
            # corr_embed = torch.cat([part_corr_embed, obj_corr_embed], dim=2)
        part_logit = self.conv_decoder(agg_output, projected_decoder_guidance)
        # part_logit = self.conv_decoder(agg_output, projected_decoder_guidance)
        return part_logit