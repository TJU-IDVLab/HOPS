...
class DINOv2Backbone(Backbone):
    def __init__(self, cfg, input_shape):
        ...
        local_repo = "/mnt/Disk16T/zjp/gsyy/code/dinov2-main"
        # 告诉 torch.hub 从本地加载
        self.dinov2 = torch.hub.load(
            local_repo,
            'dinov2_vits14',
            source='local',         # 关键：从本地而非 GitHub
            pretrained=False        # 这里设为 False，后面手动加载权重
        )
        # 再手动加载你自己的 checkpoint
        checkpoint = torch.load("/mnt/Disk16T/zjp/gsyy/code/part-catseg-main/baselines/modeling/backbone/dinoweight/dinov2_vits14_pretrain.pth", map_location="cpu")
        ...
        if "model" in checkpoint:
            ...
            sd = checkpoint  # 直接整个 dict 就是 state dict
        # 2) 加载
    ...
    def forward(self, images, need_weights=False):
        """
        ...
        embeddings = self.dinov2.forward_features(images_resized_dino, need_weights=True)
        x_norm_patchtokens = embeddings["x_norm_patchtokens"]
        if need_weights:
            attn = embeddings["attns"]
