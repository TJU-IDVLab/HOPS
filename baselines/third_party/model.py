    ...
    def forward(self, x: torch.Tensor, dense=False, need_weights=False):
        ...
        for i, resblock in enumerate(self.resblocks):
            if i == self.layers - 1 and dense:
                x = resblock.forward_dense(x)
                if need_weights:
                    attention_weights.append(None)  # forward_dense doesn't return attention weights
            else:
                if need_weights:
                    x, attn_weights = resblock(x, need_weights=True)
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, need_weights=False):
            ...
            if need_weights:
                x, attention_weights = self.transformer(x, dense, need_weights=True)
            else:
                x = self.transformer(x, dense)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, masks=None, pool_mask=None, dense=False, need_weights=False): 
        if pool_mask is not None:
            return self.visual(image.type(self.dtype), mask=pool_mask, dense=dense)
        if masks == None:
            if need_weights:
                return self.visual(image.type(self.dtype), dense=dense, need_weights=True)
            else:
                return self.visual(image.type(self.dtype), dense=dense)
        else:
            return self.visual(image.type(self.dtype), masks.type(self.dtype))
