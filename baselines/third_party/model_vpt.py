...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, need_weights=False):
        ...
        for i, resblock in enumerate(self.resblocks):
            ...
            if need_weights:
                x_attn = resblock.ln_1(x)
                attn_out, attn_weights = resblock.attn(x_attn, x_attn, x_attn, need_weights=True, attn_mask=resblock.attn_mask)
                x = x + attn_out
                attentions.append(attn_weights.clone().detach())        # [B, num_heads, L, L]
                x = x + resblock.mlp(resblock.ln_2(x))
            else:
                x = resblock(x)
        if need_weights:
        # attentions: list length=12, each [B, 12, 577, 577] 24*24 + 1
            return x, attentions
        return x
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, need_weights=False):
        ...
        if need_weights:
            x, attns = self.transformer(x, dense=dense, need_weights=need_weights)
        else:
...
class CLIP(nn.Module):
    ...
    def encode_image_with_attn(self, image, masks=None, pool_mask=None, dense=False, need_weights=False):
        if pool_mask is not None:
            return self.visual(image.type(self.dtype), mask=pool_mask, dense=dense)
        if masks == None:
            return self.visual(image.type(self.dtype), dense=dense, need_weights=need_weights)
        else:
            return self.visual(image.type(self.dtype), masks.type(self.dtype))
